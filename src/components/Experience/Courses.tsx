...
export function Courses() {
  const [dataCoursers, setDataCoursers] = useState([]);
  const [openDescription, setOpenDescription] = useState(dataCoursers[0]);
  ...
  return(
    <div className='border-violet-600 w-full h-[430px] xs:flex xs:flex-col xs:items-center'>
      ...
      <div className="flex flex-col gap-10 my-10 mx-10 xs:mx-0">
        {dataCoursers.map((item) => {
          const { id, name, description, plataform, link, skills } = item;
          return (
            <div key={id} id={id} className="flex items-center xs:w-full xs:flex-col">
              <div className="flex gap-4 w-1/2 ml-20 xs:w-auto xs:ml-8">
                <motion.button onClick={() => setOpenDescription(id)} transition={{duration: 0.3}} className={`group inset-0 w-12 h-12 p-2 border-2 border-violet-800 rounded-full bg-transparent flex items-center bg-gradient-to-r hover:bg-gradient-to-r from-violet-800 to-purple-950 cursor-pointer ease-in-out duration-500 ${openDescription !== id && "bg-none"}`}>
